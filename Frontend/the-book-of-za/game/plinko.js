    ...
    create() {
        ...
        for (var i = 1; i < 17; i++) {
            ...
            this.balls_images = this.stack.bodies.map(body => {
                return this.add.image(body.position.x, body.position.y, 'ball').setScale(0.6);
            });
        }

        var randomX = Phaser.Math.Between(340, 350);
        var ball = this.matter.add.image(randomX, -20, 'ball').setScale(0.5);
        ball.setCircle(8);
